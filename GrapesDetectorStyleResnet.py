...
style_layers = ['conv1','conv2','conv3','conv4']
...
def history_plotter(data,times):
    ...
    plt.ylabel('loss')
    plt.legend()
    plt.savefig('data//plotted_results//train//'+'train_loss_res')
    plt.show()
    ...
    plt.ylabel('accuracy(%)')
    plt.legend()
    plt.savefig('data//plotted_results//train//'+'train_acc_res')
    plt.show()
    ...
    plt.ylabel('loss')
    plt.legend()
    plt.savefig('data//plotted_results//train//'+'val_loss_res')
    plt.show()
    ...
    plt.savefig('data//plotted_results//train//'+'val_acc_res')
    plt.show()
    ...
    plt.title('Training Time')
    fig.savefig('data//plotted_results//train//'+'training_time_res')
    plt.show()
...
def main(mode):
    ...
    if mode == 'test':
        ...
        for i,elem in enumerate(n_features):
            ...
            if len(os.listdir('models//resnet//')) < len(n_features): #some of the models are not present in the models folder 
                raise FileNotFoundError("models not found") 
            
            #loading resnet
            style_extractor=Resnet(i).to(device).eval()

            #testing phase
            #loading model
            loaded_model_res=GrapesDetector(elem).to(device)
            loaded_model_res.load_state_dict(load('models//resnet//grapes_detector_resnet'+str(i)+'.pt'))
    
            #testing models
            print(f"testing model #{i}...")
            start=time.time()
            res_loss,res_acc=test_model(test_dataloader, style_extractor, loaded_model_res, loss_fn, batch_size)
            end=time.time()
        ...
        fig.savefig('data//plotted_results//test//'+'test_losses_res')
        plt.show()
        ...
        plt.ylabel('accuracy(%)')
        fig.savefig('data//plotted_results//test//'+'test_accs_res')
        plt.show()
        ...
        fig.savefig('data//plotted_results//test//'+'test_times_res')
        plt.show()
       ...
       #deleting existing res model
        for i in range(len(n_features)):
            if os.path.isfile('models//resnet//grapes_detector_resnet'+str(i)+'.pt'):
                os.remove('models//resnet//grapes_detector_resnet'+str(i)+'.pt')
                print(f"removed res model #{i}")
        ...
        for i,elem in enumerate(n_features):
            ...
            grapes_detector_res=GrapesDetector(elem).to(device)

            #defining optimizer
            optimizer= torch.optim.Adam(grapes_detector_res.parameters(),lr=learning_rate)

            #training model (with validation)
            start=time.time()
            history=train_model(train_dataloader, val_dataloader, style_extractor, grapes_detector_res, loss_fn, optimizer, batch_size, epochs)   
            end=time.time()
            train_time=np.round((end-start)/60,decimals=2)
            times.append(train_time)
            data.append(history)
            print(f"RES model #{i} Trained!")

            #saving trained model as 'model_state.pt'
            print('saving trained model') 
            with open('models//resnet//grapes_detector_resnet'+str(i)+'.pt', 'wb') as fsave: 
                save(grapes_detector_res.state_dict(), fsave)
